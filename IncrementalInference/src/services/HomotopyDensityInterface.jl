...
function setBelief!(
  ...
  belief_ = if !hasState(vari, solveKey)
    belief = defaultBelief(vari,NPBPSolver();num_kernels=100,varType=getStateKind(hode))
    state = State(solveKey, getStateKind(vari); belief, initialized = false, marginalized = false)
    setBelief!(state, hode, setinit)
    # first time this stateLabel is being added to the variable
    mergeState!(
      vari,
      state,
    )
  else
    # assume state.belief by reference is being used
