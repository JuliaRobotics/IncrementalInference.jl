...
"""
function parchDistribution(
  mkd::ApproxManifoldProducts.HomotopyDensityLive
)::DistributedFactorGraphs.HomotopyDensityDFG
  pts = getPoints(mkd)
  bw = getBW(mkd)[:, 1]
  # FIXME, ensure HomotopyDensityDFG comes out
  return manikde!(
    mkd.manifold,
    pts[1:1],
    mkd._u0;
...
function DFG.pack(mkd::ApproxManifoldProducts.HomotopyDensityLive)
  #
  DistributedFactorGraphs.HomotopyDensityDFG(mkd)
  # pts = getPoints(mkd)
...
function DFG.unpack(dtr::DistributedFactorGraphs.HomotopyDensityDFG)
  ApproxManifoldProducts.HomotopyDensityLive(dtr)
...
function Base.convert(::Type{String}, mkd::ApproxManifoldProducts.HomotopyDensity)
  #
  return JSON.write(convert(DistributedFactorGraphs.HomotopyDensityDFG, mkd))
  # packedMKD = pack(mkd)
...
function Base.convert(
  ::Type{<:H}, 
  str::AbstractString
) where {H <: ApproxManifoldProducts.HomotopyDensity}
  dtr = JSON.parse(str, DistributedFactorGraphs.HomotopyDensityDFG)
  return convert(H, dtr)
end
