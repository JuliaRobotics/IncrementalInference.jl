...
function _readBasePoint(subfg::AbstractDFG, varlabelsAP, solveKey::Symbol)
  return map(varlabelsAP) do label
    getSingleModePoint(subfg, label, solveKey)
  end
end
...
function _writeBundle!(
  ...
  for v in eachvariable(bundle)
    ...
    Σ_v = v.label in covarianceLabels ? Σ[v.range, v.range] : getSingleModeCovariance(state)
    setSingleModeBelief!(state, v.point, Σ_v)
  end
  return bundle
end
...
function solveTreeParametric!(
  ...
  csmoptions::CSMOptions = CSMOptions(; solverparams = getSolverParams(fg)),
)
  ensureSolvable!(fg)

  csmoptions.algorithm = :parametric
  ...
  if 0 < csmoptions.limititers < _needed
    @error "FIXME `limititers = $(csmoptions.limititers)` is below the ~$_needed state transitions \
`solver.iters = $(solver.iters)` sweeps need; the solve might hang."
  end

  prepareStates!(fg, NLLSSolver(), solveKey)
  ...
  solveTreePass!(fg, tree_; smtasks, csmoptions)
