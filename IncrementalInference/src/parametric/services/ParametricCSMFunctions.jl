...
function setupLinear_ParametricStateMachine(csmc::CliqStateMachineContainer)
  ...
  cliqdata.linearizer = if isempty(factorLabels)
    ...
    CliqueLinearizer(
      ...
      solveKey = csmc.csmoptions.solveKey, partition = (frontals, separators),
    )
  end
  ...
  if !isnothing(cliqdata.elimination)
    ...
    cliqdata.elimination.posterior = LinearizedBelief(
      DensityBundlePoint(
        own.layout,
        _readBasePoint(subfg, getLabelpartition(own), csmc.csmoptions.solveKey),
        zeros(n, n),
      ),
      zeros(n),
    )
...
function solveUp_ParametricStateMachine(csmc::CliqStateMachineContainer)
  ...
  relinearizeTol = csmc.csmoptions.solver isa TangentSpaceSolver ? csmc.csmoptions.solver.relinearizeTol : 0.0
  exactJacobian = csmc.csmoptions.solver isa TangentSpaceSolver ? csmc.csmoptions.solver.exactJacobian : false
  tol = csmc.csmoptions.solver isa TangentSpaceSolver ? csmc.csmoptions.solver.tol : 0.0
  cliqdata = getCliqueData(csmc.cliq)
  ...
  p_current = isnothing(cache) ?
    _readBasePoint(subfg, getLabelpartition(cliqdata.cliquelayout), csmc.csmoptions.solveKey) :
...
function solveDown_ParametricStateMachine(csmc::CliqStateMachineContainer)
  ...
  elimination = if isnothing(cache)
    ...
    eliminateCliqueFrontals!(
      cliqdata,
      _readBasePoint(subfg, getLabelpartition(cliqdata.cliquelayout), csmc.csmoptions.solveKey), childMsgs;
      exactJacobian = csmc.csmoptions.solver isa TangentSpaceSolver ? csmc.csmoptions.solver.exactJacobian : false,
    )
  else
    cache
  ...
  status = if isroot
    ...
    iters = csmc.csmoptions.solver isa TangentSpaceSolver ? csmc.csmoptions.solver.iters : 1
    tol = csmc.csmoptions.solver isa TangentSpaceSolver ? csmc.csmoptions.solver.tol : 0.0
