...
function doCliqDownSolve_StateMachine(csmc::CliqStateMachineContainer)
  ...
  if csmc.opts.multiproc
    ...
    drt = downGibbsCliqueDensity(csmc.cliqSubFg, csmc.cliq, dwnmsgs, 100, 3, false, csmc.logger)
  end
...
function attemptCliqInitDown_StateMachine(csmc::CliqStateMachineContainer)
  ...
  dwinmsgs = prepCliqInitMsgsDown!(csmc.dfg, csmc.tree, prnt, csmc.cliq, logger=csmc.logger, dbgnew=haskey(getSolverParams(csmc.dfg).devParams,:useParentFactorsInitDown)) # csmc.cliqSubFg
